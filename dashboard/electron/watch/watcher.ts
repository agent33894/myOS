import { watch, type FSWatcher } from 'fs';
import { stat } from 'fs/promises';
import { join, sep } from 'path';
import type { IpcEventMap } from '../../shared/ipc/contracts';
import { revOf } from '../documents/markdown';

type Change = IpcEventMap['files:changed'];

const FLUSH_DELAY_MS = 150;
const HIDDEN = /(^|\/)(\.[^/]*|node_modules)(\/|$)/;

let watcher: FSWatcher | null = null;

/**
 * Report changes under `root`, coalesced per path: Markdown files with their
 * new rev, and folders. A path that is gone and isn't Markdown is reported as
 * a removed folder (it may have been another kind of file; listeners reload).
 */
export function watchWorkspace(root: string | null, onChange: (change: Change) => void): void {
  watcher?.close();
  watcher = null;
  if (!root) return;

  // path -> whether the batch saw a rename (a new entry, or an editor's atomic save)
  const pending = new Map<string, boolean>();
  let timer: NodeJS.Timeout | null = null;

  const flush = () => {
    timer = null;
    const batch = [...pending];
    pending.clear();
    for (const [path, renamed] of batch) {
      const markdown = path.toLowerCase().endsWith('.md');
      stat(join(root, path)).then(
        (stats) => {
          if (stats.isDirectory()) onChange({ path, entry: 'folder', kind: renamed ? 'created' : 'updated' });
          else if (markdown && stats.isFile()) onChange({ path, entry: 'file', kind: renamed ? 'created' : 'updated', rev: revOf(stats) });
        },
        () => onChange({ path, entry: markdown ? 'file' : 'folder', kind: 'deleted' }),
      );
    }
  };

  try {
    watcher = watch(root, { recursive: true }, (event, filename) => {
      const path = filename?.split(sep).join('/');
      if (!path || HIDDEN.test(path)) return;
      pending.set(path, pending.get(path) || event === 'rename');
      if (timer) clearTimeout(timer);
      timer = setTimeout(flush, FLUSH_DELAY_MS);
    });
  } catch (error) {
    console.error('Failed to watch the folder:', error);
  }
}
