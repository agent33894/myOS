    ...
    <div className="flex flex-col gap-2">
      ...
      {propertiesOnly ? (
        <p className="px-1 py-6 text-center text-base text-text-secondary">
          The text matches the page as it is now. Only properties like status, dates, or tags differ.
        </p>
      ) : null}
...
function HistoryPanel({ target, onClose }: { target: Target; onClose: () => void }) {
  const item = useDataStore((state) => state.byPath[target.path]);
  ...
  useEffect(() => {
    ...
    void target
      .flush()
      .then(() => Promise.all([listVersions({ filePath: target.path }), read(target.path)]))
      .then(([list, file]) => {
        if (!active) return;
        setVersions(list);
  ...
  useEffect(() => {
    if (!selected) return;
    let active = true;
    void readVersion({ filePath: target.path }, selected).then(
  ...
  const restore = async () => {
    ...
    try {
      await target.flush();
      const latest = useDataStore.getState().byPath[target.path] ?? item;
      await restoreVersion(latest, selected);
      followUndo('Restored');
      onClose();
    } catch (error) {
    ...
    return (
      <div className="grid h-full place-items-center">
        <EmptyState
          ...
          description="myOS keeps a copy each time you save, at most every ten minutes."
        />
      </div>
    );
  }
...
/** Version history for one page: saved versions, what changed since each, and Restore. */
export function VersionHistoryDialog({ target, onClose }: { target: Target | null; onClose: () => void }) {
  const title = useDataStore((state) => (target ? state.byPath[target.path]?.title : undefined));
  return (
    <Dialog open={target !== null} onOpenChange={(open) => !open && onClose()}>
      ...
      >
        <DialogHeader>
          ...
          <DialogDescription>
            {title ? `“${title}” · ` : ''}A copy is kept each time you save, at most every ten minutes, for sixty days.
          </DialogDescription>
        </DialogHeader>
        <div className="min-h-0 flex-1">{target ? <HistoryPanel target={target} onClose={onClose} /> : null}</div>
      </DialogContent>
    </Dialog>
  );
}
