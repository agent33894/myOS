...
import { firstPushTarget, initWithToast, showPanel, startCommit, syncWithToast } from './actions';
...
export const gitCommands: CommandSource = ({ activePath, inRepo }) => {
  const upstream = firstPushTarget(useGitStore.getState().status);
  return [
    ...(inRepo
      ? ([
          ...
          { id: 'git.pull', group: 'Git', label: 'Pull', icon: ArrowDownToLine, keywords: 'git fetch sync rebase', run: () => syncWithToast('pull') },
          {
            id: 'git.push',
            group: 'Git',
            label: upstream ? `Push and set upstream (${upstream})` : 'Push',
            icon: ArrowUpFromLine,
            keywords: 'git sync upload',
            run: () => syncWithToast('push', Boolean(upstream)),
          },
