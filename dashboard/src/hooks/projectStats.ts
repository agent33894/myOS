...
export type ProjectHealth = 'active' | 'at-risk' | 'dormant';
...
type ProjectLike = Pick<Artifact, 'id' | 'title' | 'related' | 'updated' | 'created'> & {
  status?: Artifact['status'];
};

/** `project:` matches title or id; `related:` links count in either direction. */
export function isLinkedToProject(artifact: Artifact, project: ProjectLike): boolean {
...
export interface ProjectStats {
  ...
  doneTodos: Artifact[];
  materials: Artifact[];
  materialsByType: Array<{ type: ArtifactType; items: Artifact[] }>;
