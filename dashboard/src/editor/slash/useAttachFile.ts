...
/** Pick a file, copy it into the folder's `assets/`, and insert it (images inline, other files as a link). */
export function useAttachFile(editor: Editor, path: string) {
  const busy = useRef(false);
  return async () => {
    if (busy.current) return;
    busy.current = true;
    try {
      const result = await invoke('files:attach-asset', path);
